...
export default function TimelinePage() {
  ...
  return (
    <div className="page">
      ...
      <Panel title={t('count_events_title', { count: items.length })}>
        {items.length === 0 && <Empty message={t('no_timeline_events')} />}
          ...
          {items.map((e) => (
            <div key={e.id} className="timeline-item">
              ...
              <div className="timeline-time">
                ...
                <div className="mono">{e.time || <span className="muted">{t('time_unavailable')}</span>}</div>
              </div>
              <div className="timeline-body">
                <span className="badge type-badge">{e.type}</span>
                <div className="muted">{e.description}</div>
                <div className="small muted mono">{t('source_prefix', { source: e.source })}</div>
              </div>
            </div>
          ))}
        </div>
