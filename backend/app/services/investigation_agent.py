...
def _fast_tools(context: dict, question: str) -> InvestigationToolResult | None:
    ...
    if any(x in q for x in ("suspicious relationship", "suspicious connection", "suspicious relation", "red flag")):
        ranked = sorted(relationships, key=lambda r: (r.get("score") or 0), reverse=True)[:15]
        return InvestigationToolResult(
    ...
    if any(x in q for x in ("how many people", "number of people")):
        return InvestigationToolResult(tool="case_summary", status="ok", observations={"people": counts.get("people", 0)})

    if any(x in q for x in ("how many relationships", "number of relationships")):
        return InvestigationToolResult(tool="case_summary", status="ok", observations={"relationships": counts.get("relationships", 0)})

    if any(x in q for x in ("how many entities", "number of entities")):
        return InvestigationToolResult(tool="case_summary", status="ok", observations={"entities": counts.get("entities", 0)})

    if any(x in q for x in ("compare", "across the selected cases", "between these cases")):
        by_case = {case["id"]: {"name": case["name"], "relationships": []} for case in context.get("cases", [])}
...
def run_investigation_tools(db, user, case_ids: list[str], question: str, context: dict | None = None) -> InvestigationToolResult:
    ...
    if selected_case and any(x in q for x in ("degree", "centrality", "pagerank", "page rank", "betweenness", "closeness", "community", "network", "bridge", "cluster", "shortest path", "graph analysis")):
        try:
            result = analyze_case(db, user, selected_case)
            return InvestigationToolResult(tool="graph_metrics", status="ok", case_id=selected_case, observations=result)
...
def _sql_graph_observations(context: dict) -> dict[str, Any]:
    ...
    for r in rels:
        ...
        if not aid or not bid:
            continue
        names[aid], names[bid] = a.get("name") or aid, b.get("name") or bid
    ...
    people = [
        {"entity_id": pid, "name": names.get(pid, pid), "degree": degree.get(pid, 0), "weighted_degree": round(weighted.get(pid, 0.0), 6)}
        for pid in sorted(degree, key=lambda p: (-weighted.get(p, 0.0), -degree.get(p, 0), names.get(p, p)))
    ]
    return {"mode": "sql_relationship_fallback", "ranked_people": people[:50], "degree_centrality": people[:50], "note": "Neo4j GDS was unavailable, so these are deterministic SQL relationship metrics."}
